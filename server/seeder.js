...
// ✨ NOTE: Ensure your ./models/Product.js schema now includes: description: { type: String, required: false }
...
const seedDatabase = async () => {
    ...
    try {
        ...
        console.log(`    > Admin '${savedAdmin.fname} ${savedAdmin.lname}' created.`);

        // 2. CREATE SHOPS for the User
        console.log('🏪 Step 2: Creating 2 shops for the user...');
        const shop1 = await Shop.create({ name: 'Adhikari General Store', address: 'New Baneshwor, Kathmandu', owner: savedUser._id });
        const shop2 = await Shop.create({ name: 'Pokhara Trade Center', address: 'Lakeside, Pokhara', owner: savedUser._id });
        ...
        for (const shop of allShops) {
            ...
            const numberOfProductsToCreate = 25;
            for (let i = 0; i < numberOfProductsToCreate; i++) {
                const category = getRandomElement(productCategories);
                const productName = faker.commerce.productName();
                // ✨ ADDED: Generate a realistic product description using faker.
                const productDescription = faker.commerce.productDescription();
                const purchasePrice = parseFloat(faker.commerce.price({ min: 100, max: 20000, dec: 0 }));
                ...
                data.products[shopIdStr].push(
                    await Product.create({
                        name: productName,
                        category: category,
                        description: productDescription, // ✨ ADDED: The new description field.
                        purchasePrice,
                        sellingPrice,
                        quantity: 0,
                        reorderLevel: 10,
             ...
             // ✨ UPDATED: The log message now reflects that descriptions are created.
            console.log(`    > ${data.products[shopIdStr].length} products with categories and descriptions created for ${shop.name}`);
