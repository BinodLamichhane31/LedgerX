...
const SalesStatusBadge = ({ sale }) => {
    ...
    if (sale.status === 'CANCELLED') {
        ...
        switch (sale.paymentStatus) {
            ...
            case 'DUE': config = { text: 'Due', className: 'bg-rose-100 text-rose-800' }; break;
            default: break;
        }
    }
...
const SaleDetailHeader = ({ sale, onPrint, onCancelClick, onPaymentClick, canCancel, canRecordPayment }) => (
    <div className="flex flex-col items-start justify-between gap-4 pb-6 border-b print:hidden md:flex-row md:items-center">
        ...
        <div className="flex items-center flex-shrink-0 gap-2">
            ...
            {canRecordPayment && (
                <button onClick={onPaymentClick} className="inline-flex items-center gap-2 px-3 py-2 text-sm font-medium text-white bg-indigo-600 border border-transparent rounded-lg shadow-sm hover:bg-indigo-700">
                    <Banknote size={16} /> Record Payment
                </button>
            )}
            {canCancel && (
                <button onClick={onCancelClick} className="inline-flex items-center gap-2 px-3 py-2 text-sm font-medium text-white bg-rose-600 border border-transparent rounded-lg shadow-sm hover:bg-rose-700">
...
const SaleFinancials = ({ sale }) => (
    <div className="flex justify-end mt-6">
        <div className="w-full max-w-sm space-y-2 text-sm">
            ...
            <div className="flex justify-between pt-2 mt-2 text-base font-bold border-t"><span className="text-slate-900">Grand Total:</span><span className="text-slate-900">{sale.grandTotal.toLocaleString('en-IN', { style: 'currency', currency: 'NPR' })}</span></div>
            <div className="flex justify-between"><span className="text-slate-600">Amount Paid:</span><span className="font-medium text-emerald-600">{sale.amountPaid.toLocaleString('en-IN', { style: 'currency', currency: 'NPR' })}</span></div>
            <div className="flex justify-between p-3 text-lg font-bold rounded-lg text-indigo-800 bg-indigo-50"><span>Amount Due:</span><span>{sale.amountDue.toLocaleString('en-IN', { style: 'currency', currency: 'NPR' })}</span></div>
        </div>
    </div>   
);
    ...
    return (
        <div className="min-h-screen p-4 bg-slate-50 sm:p-6 lg:p-8 print:bg-white print:p-0">
            <div className="max-w-4xl mx-auto print:max-w-none">
                <Link to="/sales" className="inline-flex items-center gap-2 mb-4 text-sm font-medium transition-colors text-slate-500 print:hidden hover:text-indigo-600">
                    <ChevronLeft size={16} />
